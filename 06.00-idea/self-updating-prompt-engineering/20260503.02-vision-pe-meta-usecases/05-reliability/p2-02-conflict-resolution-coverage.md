...
Verify the **conflict-resolution table is complete and exercised**. Every rationale pair that has produced a real-world tension SHOULD have a precedence rule. Pairs without a rule are latent oscillation risks (see [p1-01-loop-stability-audit](p1-01-loop-stability-audit.md)) and latent disagreement risks across reviews.
...
D29 (loop-stability) — secondary (oscillation often stems from missing precedence)
...
- [p1-01-loop-stability-audit](p1-01-loop-stability-audit.md) — oscillation detection feeds this UC's input
- [p2-01-autonomy-calibration-audit](p2-01-autonomy-calibration-audit.md) — calibration recommendations and conflict-rule recommendations are batched in the same vision-update cycle
