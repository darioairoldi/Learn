...
|---|---|
| `--dim craftmanship` | Focus on D5 (boundaries) + D14 (craftmanship) |
...
**Dimensions covered:** D5 (boundaries), D14 (craftmanship)

**Checks performed:**
- D5: Boundary completeness — ≥5/2/3 (exemplary bar); each boundary item is testable and actionable; boundaries cross-reference tools and responsibilities
- D14: N-1 separation — rule-bearing sections use `Rule`/`Rationale`/`Example` labeled blocks; naming follows kebab-case convention; content follows progressive disclosure (summary → detail → reference); section ordering matches type-specific template; emoji H2 headings present (for articles); metadata comment block at file end

## Reliability analysis

| Factor | Assessment |
|---|---|
| **Determinism** | ⚠️ D5 boundary count is deterministic; actionability assessment requires LLM. D14 N-1 label presence is deterministic; naming/ordering is pattern-matchable |
