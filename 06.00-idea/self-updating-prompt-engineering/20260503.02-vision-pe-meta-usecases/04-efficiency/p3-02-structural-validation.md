...
|---|---|
| `--dim structural` | Focus on D1–D4 (metadata, references, tokens, tools) |
| `--dim metadata` | D1 only (YAML validation) |
| `--dim references` | D2 only (reference integrity) |
...
**Checks performed:**
- D1: Are `goal:`, `scope:`, `boundaries:`, `rationales:`, `version:` present in YAML?
- D2: Do all `📖` references, markdown `[text](path)` links, and `/slash-command` references resolve to existing files?
- D3: Is the artifact within its type-specific token budget (from 01.06)?
- D4: Does `agent: plan` have only read-only tools? Does `agent: agent` include write tools? Is tool count 3-7?
...
| **False negatives** | Near-zero for D1-D4; possible for D2 if a reference uses an unexpected pattern |
