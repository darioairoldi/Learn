...
|---|---|
| `--dim optimize` | Efficiency dimension group including `D3-token-budget`, `D20-token-chain` |
| `--dim token-budget` | Focus on `D3-token-budget` (per-artifact budget) |
...
**Checks performed:**
- `D3-token-budget`: Count tokens in the artifact body. Compare against type-specific limits from 01.06 (context: 2,500; instruction: 1,500; simple prompt: 1,500; orchestrator: 2,500; meta-prompt: 3,000)
- `D20-token-chain`: Enumerate all `📖` references and `context_dependencies` for an agent/prompt. Sum the token counts of all referenced guidance files. Report: total chain cost, percentage of context window consumed, consumer burden (files loaded but not referenced in process phases)

## Reliability analysis

| Factor | Assessment |
|---|---|
| **Determinism** | ✅ `D3-token-budget` fully deterministic (count tokens). `D20-token-chain` fully deterministic (sum referenced file sizes) |
...
| Factor | Assessment |
|---|---|
| **Catches real issues** | ✅ `D3-token-budget` catches oversized files. `D20-token-chain` catches context window saturation |
...
| **Unique value** | `D20-token-chain` (chain analysis) is the ONLY check that assesses token cost at the consumer level, not the producer level |

**Effectiveness score: MEDIUM-HIGH** — `D3-token-budget` is straightforward; `D20-token-chain` provides unique insight into whether the guidance chain is practically usable.

## Efficiency analysis

| Factor | Assessment |
|---|---|
| **Token cost** | `D3-token-budget`: near-zero (count lines). `D20-token-chain`: LOW (sum file sizes, no LLM needed) |
| **Model routing** | Small model or deterministic tool — no LLM reasoning needed |
| **Time** | `D3-token-budget`: <1s. `D20-token-chain`: 5-10s (reads multiple files) |
| **Recommended frequency** | `D3-token-budget`: on every change. `D20-token-chain`: during scheduled reviews; after adding new `📖` references |
