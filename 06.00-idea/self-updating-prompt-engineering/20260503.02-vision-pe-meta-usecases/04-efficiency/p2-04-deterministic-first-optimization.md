...
|---|---|
| `--dim deterministic-first` | Focus on D21 specifically |
| `--dim optimize` | Broader efficiency group including D21 |
...
**Critical constraint**: Deterministic processing AUGMENTS reasoning — it never replaces it. Whatever the deterministic portion doesn't cover, LLM reasoning still handles at full scope. D21 proposes splits, not removals.
...
**Example output:**
```markdown
## D21: Deterministic-first optimization report
