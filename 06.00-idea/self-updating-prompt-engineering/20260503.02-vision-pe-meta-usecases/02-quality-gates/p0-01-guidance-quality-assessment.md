...
|---|---|
| `--dim quality` | Limits to D6–D11 guidance quality dimensions |
...
**Checks performed:**
- D6: Do rules within the file contradict each other? Do rules contradict rules in files this one references?
- D7: Is each rule defined in exactly one place? `grep_search` for key phrases across context files
- D8: Where rules could conflict, is the priority matrix entry present? Are priorities explicit?
- D9: LLM agreement test — two independent passes produce the same interpretation of each rule
- D10: Scan agent behaviors that lack backing rules — are there agents that do things no rule governs?
- D11: Can each rule be translated into a boolean pass/fail check?

## Reliability analysis

| Factor | Assessment |
|---|---|
| **Determinism** | ⚠️ D7, D8 partially deterministic (grep-based); D6, D9, D10, D11 require LLM judgment |
...
| **Consistency** | ⚠️ D9 (clarity 2-pass) may produce different results across runs due to LLM variability |

**Reliability score: MEDIUM** — LLM-dependent dimensions introduce variability. Mitigated by: (1) running D9 with temperature=0, (2) accepting findings only when both passes agree, (3) using D7/D8 deterministic checks as anchors.
...
| Factor | Assessment |
|---|---|
| **Token cost** | MEDIUM-HIGH — D9 requires two full LLM passes per rule; D10 requires cross-referencing agents against rules |
| **Model routing** | D7, D8: standard model. D6, D9, D10, D11: reasoning model |
...
- Single-file quality hardening rerun (D9/D11 focus): [99.00-temp/20260521-uc02-guidance-quality-assessment-01.07-rerun-3.md](../../../../99.00-temp/20260521-uc02-guidance-quality-assessment-01.07-rerun-3.md)
- Folder-level completeness rerun (D10): [99.00-temp/20260521-context-review-completeness-folder-rerun.md](../../../../99.00-temp/20260521-context-review-completeness-folder-rerun.md)
