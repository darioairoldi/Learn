...
|---|---|
| `--dim context-optimization` | `D22-context-optimization` context-specific optimization |
| `--dim context-health` | Combined `D6-consistency` through `D11-actionability` + `D22-context-optimization` for full context layer assessment |
...
Reviews the context file SET as a system — checking organizational health, layer correctness, granularity, category mapping integrity, consumer load patterns, and topic coherence. This is distinct from `D6-consistency` through `D11-actionability` which check individual file quality; `D22-context-optimization` checks whether the files are optimally organized for their consumers.

Boundary note: this use case is optimization-scoped (`D22-context-optimization`) and does not replace full context quality lifecycle analysis. Full lifecycle handling of consistency, redundancy, staleness/source verification, completeness, and integration decisions is defined in UC-22.
...
| Dimension | Scope | Overlaps with `D22-context-optimization`? |
|---|---|---|
| `D6-consistency` | Within-file contradictions | No — `D22-context-optimization` checks organization, not rule consistency |
| `D7-non-redundancy` | Rule duplication across files | Partial — `D22-context-optimization`'s topic coherence may detect overlap, but `D7-non-redundancy` checks at rule level |
| `D10-completeness` | Missing rules | No — `D22-context-optimization` checks file-level gaps, `D10-completeness` checks rule-level gaps |
| `D12-staleness` | Timestamp freshness | Partial — `D22-context-optimization`'s freshness distribution uses the same timestamps but looks at the set pattern |
| `D20-token-chain` | Consumer loading cost | Partial — `D22-context-optimization`'s consumer load analysis overlaps with `D20-token-chain` but adds orphan detection and consumer count |

**Key distinction**: `D6-consistency` through `D11-actionability` answer "is this file's CONTENT good?" `D22-context-optimization` answers "is this file's ORGANIZATION good?" Both are needed — a well-written file in the wrong layer or with zero consumers is still a problem.
