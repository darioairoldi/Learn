...
**Dimensions covered:** All applicable dimensions + cross-dependency checks (`D17-cross-coherence`, `D20-token-chain`)
...
4. Cross-dependency coherence (`D17-cross-coherence`): Do loaded context files contradict each other?
5. Adherence verification (`D16-adherence`): Does the target implement all rules from its dependencies?
6. Token chain analysis (`D20-token-chain`): Total loading cost of the target's guidance set
