...
3. [Infrastructure Overview Page](#3-infrastructure-overview-page)
4. [Per-Environment Page](#4-per-environment-page)
5. [Azure Icon and Visual Conventions](#5-azure-icon-and-visual-conventions)
6. [Sensitive Value Handling](#6-sensitive-value-handling)
...
3. **Naming convention table** — document the pattern for all resource types:
   ...
   |---------------|---------|---------|
   | App Service | `{prefix}-{env}-{service}` | `abb-test-apidevice` |
   | SQL Server | `{prefix}-{env}-sql` | `abb-prod-sql` |
   | _(etc.)_ | | |
