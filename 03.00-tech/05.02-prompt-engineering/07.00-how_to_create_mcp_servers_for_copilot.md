...
While the previous articles in this series covered **consuming** Copilot customizations, this article focuses on **creating** your own MCP servers—building the server-side components that provide new capabilities to Copilot Chat.
...
- [🏗️ MCP server architecture](#️-mcp-server-architecture)
...
- [💻 Implementation: C# (.NET)](#-implementation-c-net)
- [💻 Implementation: Python](#-implementation-python)
- [⚙️ Configuration and registration](#️-configuration-and-registration)
- [🧪 Testing and debugging](#-testing-and-debugging)
- [🚀 Deployment patterns](#-deployment-patterns)
- [⚠️ Common pitfalls](#️-common-pitfalls)
- [💡 Decision framework](#-decision-framework)
- [🧩 MCP Apps — Rich UI in chat (experimental)](#-mcp-apps--rich-ui-in-chat-experimental)
  - [Architecture](#architecture)
  - [How it works](#how-it-works)
  - [The promise pattern — forcing chat to wait](#the-promise-pattern--forcing-chat-to-wait)
...
Think of it as a <mark>**universal adapter**</mark>—rather than building custom integrations for each AI assistant, you build one MCP server that works with any MCP-compatible client.
...
| **<mark>Cross-platform</mark>** | ✅ Any MCP client | ✅ VS Code, CLI, coding agent | ❌ VS Code only | ❌ VS Code only |

### When to build an MCP server

**✅ Build an MCP server when you need to:**

- <mark>**Query external systems**</mark> — Databases, APIs, internal services
- <mark>**Perform complex computations**</mark> — Data processing, analysis, transformations
- <mark>**Access live data**</mark> — Real-time metrics, monitoring, dashboards
- <mark>**Enforce business logic**</mark> — Validation rules, compliance checks
- <mark>**Integrate proprietary tools**</mark> — Internal tooling, legacy systems
- <mark>**Share capabilities across projects**</mark> — Reusable tooling for teams
...
┌─────────────────────────────────────────────────────────────────┐
│  1. INITIALIZATION                                              │
│  ├── Client discovers server (from mcp.json configuration)      │
...
│  ├── Server: Lists tools with JSON Schema definitions           │
│  └── Server: Lists resources and prompt templates               │
└────────────────┬────────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────────┐
│  3. RUNTIME OPERATION                                           │
│  ├── Client sends tool invocation requests                      │
...
┌─────────────────────────────────────────────────────────────────┐
│  4. SHUTDOWN                                                    │
│  └── Client terminates server process                           │
└─────────────────────────────────────────────────────────────────┘
```
...
<mark>**All MCP servers run as separate processes**</mark>—they are **never** loaded in-process into the AI client (like VS Code or Claude Desktop). This is a fundamental architectural decision:

```
┌──────────────────────────────────────────────────────────────────┐
│  MCP HOST (e.g., VS Code, Claude Desktop)                        │
│  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐     │
...
| Benefit | Explanation |
|---------|-------------|
| **<mark>Language independence</mark>** | Write servers in any language—the client doesn't care |
...
|-----------|--------------|------------------|
| **<mark>stdio</mark>** | Client spawns server as subprocess, communicates via stdin/stdout | ✅ All SDKs |
| **<mark>Streamable HTTP</mark>** | Server runs as HTTP endpoint, client connects via POST/SSE | ✅ All SDKs |

<mark>**Key insight**: The client doesn't know or care what language your server is written in.</mark> A TypeScript client can connect to a Python server, and a C# host can spawn a TypeScript server—the JSON-RPC 2.0 messages are identical.
...
| **Notifications** | ✅ | ✅ | ✅ | Real-time updates to client |
| **Progress reporting** | ✅ | ✅ | ✅ | Long-running operation status |
| **Structured output** | ✅ | ✅ | ✅ | Typed tool responses (spec 2025-06-18) |
...
1. **<mark>Name</mark>** — Unique identifier (e.g., `query_database`)
2. **<mark>Description</mark>** — What the tool does (helps AI decide when to use it)
3. **<mark>Input Schema</mark>** — JSON Schema defining required and optional parameters
4. **<mark>Handler</mark>** — Function that executes when the tool is called
...
# ⚙️ Configuration and registration
...
// ✅ Focused tool with sensible defaults
inputSchema: {
  properties: {
    source: { type: "string" },
    destination: { type: "string" }
...
│
├─ Just coding standards/rules?
│  └─ Use instruction files (.instructions.md)
│
├─ Reusable task with user input?
│  └─ Use prompt files (.prompt.md)
│
├─ Persistent AI persona with tool restrictions?
│  └─ Use agent files (.agent.md)
│
├─ Cross-platform workflow with scripts/templates?
│  └─ Use skill files (SKILL.md)
│
├─ Need to call external APIs or databases?
│  └─ Build an MCP server ✅
│
...
- <mark>**Custom tool integrations**</mark> — Connect to any API, database, or service
- <mark>**Real-time data access**</mark> — Provide live metrics, logs, and status information
- <mark>**Business logic enforcement**</mark> — Implement validation, compliance, and domain rules
- <mark>**Cross-platform compatibility**</mark> — Work with any MCP-compatible AI assistant

**Key takeaways:**

1. **Start simple** — Build one tool, test it thoroughly, then expand
2. **Write clear descriptions** — The AI uses them to decide when to call your tools
3. **Use stderr for debugging** — Never write to stdout
4. **Handle errors gracefully** — Return structured error messages
5. **Test with MCP Inspector** — Verify protocol compliance before deployment
...
**[Model Context Protocol Specification](https://spec.modelcontextprotocol.io/)** 📘 [Official]  
...
**[MCP Architecture Overview](https://modelcontextprotocol.io/docs/concepts/architecture)** 📘 [Official]  
...
**[MCP Transports Documentation](https://modelcontextprotocol.io/docs/concepts/transports)** 📘 [Official]  
...
**[MCP Python SDK](https://github.com/modelcontextprotocol/python-sdk)** 📘 [Official]  
Official Python SDK for building MCP servers. Includes FastMCP high-level API and low-level protocol access.

**[MCP C# SDK](https://github.com/modelcontextprotocol/csharp-sdk)** 📘 [Official]  
...
**[MCP SDKs Overview](https://modelcontextprotocol.io/docs/sdk)** 📘 [Official]  
Complete list of official SDKs (TypeScript, Python, Go, Kotlin, Swift, Java, C#, Ruby, Rust, PHP) with links to documentation.

**[MCP Inspector](https://github.com/modelcontextprotocol/inspector)** 📘 [Official]  
Debugging tool for MCP servers. Essential for development and troubleshooting.

**[MCP Server Registry](https://github.com/modelcontextprotocol/registry)** 📘 [Official]  
Community registry of published MCP servers. Browse for inspiration or submit your own.

**[VS Code MCP Documentation](https://code.visualstudio.com/docs/copilot/chat/mcp-servers)** 📘 [Official]  
...
- [01. How GitHub Copilot uses markdown and prompt folders](./02-getting-started/01.00-how_github_copilot_uses_markdown_and_prompt_folders.md) — Overview of all customization types
- [03. How to structure content for prompt files](./03.00-how_to_structure_content_for_copilot_prompt_files.md) — Referencing MCP tools in prompts
- [04. How to structure content for agent files](./04.00-how_to_structure_content_for_copilot_agent_files.md) — Configuring MCP tools for agents
- [06. How to structure content for skill files](./06.00-how_to_structure_content_for_copilot_skills.md) — Cross-platform workflows
...
article_metadata:
  filename: "07.00-how_to_create_mcp_servers_for_copilot.md"
  word_count: ~2800
  created: "2026-01-20"
