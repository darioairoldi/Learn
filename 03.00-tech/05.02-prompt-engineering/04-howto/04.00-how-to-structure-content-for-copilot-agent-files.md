...
- [?? Understanding Custom Agents](#understanding-custom-agents)
- [?? AGENTS.md vs .agent.md: Understanding the Difference](#agentsmd-vs-agentmd-understanding-the-difference)
- [?? How Agents Differ from Prompts and Instructions](#how-agents-differ-from-prompts-and-instructions)
- [?? Agent File Structure](#agent-file-structure)
- [?? Designing Agent Personas](#designing-agent-personas)
- [?? Agent Interactions and Handoffs](#agent-interactions-and-handoffs)
- [?? Subagent Configuration](#subagent-configuration)
- [?? Tool Configuration for Agents](#tool-configuration-for-agents)
- [?? Composing Agents with Prompts and Instructions](#composing-agents-with-prompts-and-instructions)
- [?? Decision Framework: When to Use Each File Type](#decision-framework-when-to-use-each-file-type)
- [?? Testing and Validating Agents](#testing-and-validating-agents)
- [?? Conclusion](#conclusion)
- [?? References](#references)
- [?? Appendix: Unified Agent Architecture (v1.107+)](#appendix-unified-agent-architecture-v1107)
...
|---------|-------------|-----------|----------|
| **<mark>Local</mark>** | Interactive agent in current workspace | None�modifies workspace directly | Quick tasks, interactive refinement |
...
| **<mark>Cloud</mark>** | Runs on GitHub infrastructure, creates PRs | Full�new branch and PR | Large changes, async collaboration |
...
For comprehensive coverage of Agent HQ, session management, and work tree isolation workflows, see [?? Appendix: Unified Agent Architecture](#appendix-unified-agent-architecture-v1107).

## ?? AGENTS.md vs .agent.md: Understanding the Difference
...
| **Handoffs** | ? Workflow transitions | ? Not supported |
| **Creates PRs** | Only in cloud context | ? Always (coding agent creates PRs) |

### AGENTS.md: Instructions for Async Coding Agent

<mark>`AGENTS.md`</mark> provides instructions to the <mark>**GitHub Copilot Coding Agent**</mark>�an asynchronous AI agent that works on issues in the background and creates pull requests. This is the agent you invoke via GitHub.com ("Ask Copilot to work on this issue") or via the "Continue in Cloud" option in VS Code.
...
+-- src/
�   +-- components/
�   �   +-- AGENTS.md      # Component-specific instructions (overrides root)
�   +-- api/
...
+-- .github/
�   +-- agents/
�   �   +-- planner.agent.md           # VS Code planning persona
�   �   +-- security-review.agent.md   # VS Code security reviewer
�   �   +-- implementation.agent.md    # VS Code implementation persona
�   +-- instructions/
�       +-- coding-standards.instructions.md  # Applied to all contexts
```
...
> **?? Note**: For detailed guidance on Agent Skills, see [How to Structure Content for GitHub Copilot Skills](./06.00-how-to-structure-content-for-copilot-skills.md).
...
user-invokable: true          # Set false for subagent-only agents
disable-model-invocation: false # Set true to prevent auto-invocation
agents: ['agent1', 'agent2']   # Restrict available subagents (or * for all)
handoffs:
...
| `user-invokable` | Boolean | No | Controls whether the agent appears in the agents dropdown (default: `true`). Set to `false` for agents that should only be accessible as subagents or programmatically |
| `disable-model-invocation` | Boolean | No | Prevents the agent from being auto-invoked as a subagent by other agents (default: `false`). Use when you want manual-only subagent invocation |
| `agents` | Array | No | List of agent names available as subagents. Use `*` to allow all agents, or `[]` to prevent any subagent use. When specified, ensure `runSubagent` is included in `tools` |
...
| `infer` | Boolean | **Deprecated.** Use `user-invokable` and `disable-model-invocation` instead. Previously, `infer: true` (default) made the agent both visible in the picker and available as a subagent; `infer: false` hid it from both. The new fields give independent control |
...
Subagents allow an agent to delegate tasks to other agents programmatically via the `runSubagent` tool. Three frontmatter properties control subagent visibility and invocation:

### Controlling agent visibility

### `user-invokable`

Controls whether the agent appears in the agents dropdown picker. Set to `false` for agents that should only run as subagents, never selected directly by users:

```yaml
---
name: code-validator
description: Internal validation subagent
user-invokable: false   # Hidden from picker, subagent-only
tools: ['codebase', 'search']
---
You validate code changes against project standards...
```

### `disable-model-invocation`

Prevents the model from automatically invoking this agent as a subagent. Use when you want the agent to be available only through explicit user or coordinator invocation:

```yaml
---
name: deployment-agent
description: Deploys to production
disable-model-invocation: true  # Only invoked explicitly, never auto-selected
tools: ['terminal', 'editor']
---
You handle production deployments...
```

### Combining the properties

The table below shows how `user-invokable` and `disable-model-invocation` interact:

| `user-invokable` | `disable-model-invocation` | In picker? | Auto-subagent? | Use case |
|---|---|---|---|---|
| `true` (default) | `false` (default) | ? Yes | ? Yes | Normal agent |
| `false` | `false` | ? No | ? Yes | Background worker subagent |
| `true` | `true` | ? Yes | ? No | User-only agent (no auto-delegation) |
| `false` | `true` | ? No | ? No | Manually invoked subagent only |

> ?? **Deprecation:** The `infer` property previously combined both behaviors. `infer: true` was equivalent to `user-invokable: true` + `disable-model-invocation: false`. `infer: false` was equivalent to `user-invokable: false` + `disable-model-invocation: true`. Migrate to the new properties for independent control.

### Restricting available subagents with `agents`

The `agents` property controls which subagents a coordinator can invoke:

```yaml
---
name: orchestrator
description: Coordinates the code review workflow
tools: ['runSubagent', 'codebase', 'search']
agents: ['code-validator', 'security-reviewer', 'test-runner']
---
You coordinate code reviews by delegating to specialized subagents...
```

| Value | Behavior |
|-------|----------|
| `agents: ['name1', 'name2']` | Only listed agents can be invoked as subagents |
| `agents: ['*']` | All available agents can be used as subagents |
| `agents: []` | No subagent invocation allowed |
| *(omitted)* | All agents available (same as `*`) |

> ?? **Note:** When using the `agents` property, ensure `runSubagent` is included in your `tools` list.

For complete coverage of subagent orchestration patterns (coordinator/worker, multi-perspective review, parallel execution), see [How to Design Subagent Orchestrations](./11.00-how-to-design-subagent-orchestrations.md).

## ?? Tool configuration for agents

> **Comprehensive tool documentation has been consolidated into a dedicated article.** For the complete tool architecture�YAML capability declarations, runtime tool catalog, token costs, selection strategies by agent role, and practical patterns�see [How to Leverage Tools in Prompt Orchestrations](./09.50-how-to-leverage-tools-in-prompt-orchestrations.md).

Here's a quick reference of the key concepts specific to agent files:
...
Nine built-in YAML capabilities are available: `codebase`, `editor`, `filesystem`, `fetch`, `web_search`, `search`, `usages`, `problems`, `changes`. Three predefined tool sets group them: `#edit`, `#search`, `#reader`. MCP tools use the `@server-name` prefix (e.g., `@github/*`).

Restrict tools to match each agent's role�read-only agents shouldn't have `editor`, security-sensitive agents shouldn't have `fetch` or `web_search`. For detailed selection strategies and safety patterns, see the [tool selection strategy](./09.50-how-to-leverage-tools-in-prompt-orchestrations.md#-tool-selection-strategy) and [tool restrictions](./09.50-how-to-leverage-tools-in-prompt-orchestrations.md#?-tool-restrictions-and-safety) sections.
...
? Workflow needs bundled resources (templates, checklists, scripts)  
? Portability across projects and platforms is important  
? Task is self-contained with clear inputs and outputs  
? Multiple resources need to be loaded together  
? You want to share the workflow via agentskills.io registry  
...
> **?? For detailed Skills guidance**: See [How to Structure Content for GitHub Copilot Skills](./06.00-how-to-structure-content-for-copilot-skills.md)
...
| > 2,000 tokens | 3+ seconds | ?? Very High | ?? Refactor required |
...
1. **Peer review** � Have a colleague test the agent independently
2. **Edge cases** � Test with unusual or adversarial inputs
3. **Documentation** � Update team docs with agent purpose and usage
4. **Version note** � Add changelog entry for significant agent changes
...
- [.copilot/context/00.00-prompt-engineering/01.01-context-engineering-principles.md](../../.copilot/context/00.00-prompt-engineering/01.01-context-engineering-principles.md) - Core context engineering principles
...
- **[Use Prompt Files in VS Code](https://code.visualstudio.com/docs/copilot/customization/prompt-files)** `[?? Official]`  
...
- **[Use Custom Instructions in VS Code](https://code.visualstudio.com/docs/copilot/customization/custom-instructions)** `[?? Official]`  
...
- **[Use Tools in Chat](https://code.visualstudio.com/docs/copilot/chat/chat-tools)** `[?? Official]`  
...
- **[The GitHub Copilot customization stack](../01-overview/01.00-the-github-copilot-customization-stack.md)**  
  *Series overview mapping every customization mechanism�prompts, instructions, agents, skills, hooks, MCP, models, and more. Start here to understand how all the pieces fit together.*

- **[How to Name and Organize Prompt Files](../02-getting-started/02.00-how-to-name-and-organize-prompt-files.md)**  
...
- **[Awesome GitHub Copilot - Custom Agents Examples](https://github.com/github/awesome-copilot/tree/main)** `[?? Community]`  
...
article_metadata:
  filename: "04.00-how-to-structure-content-for-copilot-agent-files.md"
  created: "2025-12-10T00:00:00Z"
  last_updated: "2025-12-24T00:00:00Z"
  series: "GitHub Copilot Prompt Engineering"
  article_number: 4
  changelog: "04.00-how_to_structure_content_for_copilot_agent_files.changelog.md"
  related_articles:
