...
- [?? Compose Clear and Structured Content](#compose-clear-and-structured-content)
...
- [?? Testing and Validating Prompts](#testing-and-validating-prompts)
...
- <mark>**agent**</mark>: Sets the chat mode (`ask`, `edit`, `agent` or a custom agent name). When referencing a custom agent, the prompt inherits that agent's default tools and behavior. See [How to Structure Content for Copilot Agent Files](./04.00-how-to-structure-content-for-copilot-agent-files.md) for details on custom agents.
...
## ?? Compose Clear and Structured Content
...
- ?? **Visual Studio**: Limited support; check version documentation
...
|---------|----------------|----------------------|
| **Chat Variables** | ? Full (`${workspaceFolder}`, `${file}`, etc.) | ?? Limited support |
| **File References** | ? `#file:path` syntax | ?? Check version docs |
| **Tools** | ? Full (`#codebase`, `#fetch`, `#web`) | ?? Partial support |
| **MCP Servers** | ? Preview (1.106+) | ? Not supported |
| **Tool Control** | ? `tools` YAML field | ?? Limited |
...
**For comprehensive tool documentation**, see [How to Leverage Tools in Prompt Orchestrations](./09.50-how-to-leverage-tools-in-prompt-orchestrations.md) which provides the complete tool architecture, runtime tool catalog, token costs, selection strategies, and practical patterns.
...
- <mark>**Custom agents**</mark>: For reusable personas that multiple prompts can reference, create `.agent.md` files in `.github/agents/`. See [How to Structure Content for Copilot Agent Files](./04.00-how-to-structure-content-for-copilot-agent-files.md) for detailed guidance on agent design.
...
**What Are Tokens?**
- Tokens are the basic units that language models process (roughly 1 token � 4 characters or 0.75 words)
...
| 2,500-4,000 tokens | 15-25 seconds | 60-75% | ?? Use sparingly |
| > 4,000 tokens | 25+ seconds | < 60% | ?? Refactor required |
...
1. **Peer test** � Have another team member run the prompt without guidance
2. **Document** � Add usage examples to your team's prompt catalog
3. **Changelog** � Note significant changes in commit messages
4. **Monitor** � Collect feedback after rollout, iterate as needed
...
**? Solution: Focus on the happy path, reference instructions for standards:**
```markdown
---
name: create-api-endpoint
tools: [fetch]
...
**? Solution: Use valid agents and tools:**
```yaml
---
name: refactor-code
...
**? Solution: Use variables and pattern matching:**
```markdown
---
name: update-config
---
...
**? Solution: Use variable substitution (VS Code 1.99+):**
```yaml
---
name: create-react-component
description: "Generate React component with TypeScript"
...
**Usage:** `/create-react-component Button` ? substitutes `${componentName}` with `Button`
...
**? Solution: Cross-IDE compatible prompt:**
```yaml
---
name: analyze-codebase
...
validations:
  consistency_review:
    ...
    gaps_addressed: 3
article_metadata:
  filename: "03.00-how-to-structure-content-for-copilot-prompt-files.md"
  last_updated: "2025-12-24T00:00:00Z"
  changelog: "03.00-how_to_structure_content_for_copilot_prompt_files.changelog.md"
---
-->
