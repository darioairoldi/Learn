...
- [ðŸ¤– Understanding Custom Agents](#-understanding-custom-agents)
- [ï¿½ AGENTS.md vs .agent.md: Understanding the Difference](#-agentsmd-vs-agentmd-understanding-the-difference)
- [ðŸ”„ How Agents Differ from Prompts and Instructions](#-how-agents-differ-from-prompts-and-instructions)
- [ðŸ“‹ Agent File Structure](#-agent-file-structure)
- [ðŸŽ¯ Designing Agent Personas](#-designing-agent-personas)
- [ðŸ”— Agent Interactions and Handoffs](#-agent-interactions-and-handoffs)
- [ðŸ¤– Subagent Configuration](#-subagent-configuration)
- [âš™ï¸ Tool Configuration for Agents](#ï¸-tool-configuration-for-agents)
- [ðŸ§© Composing Agents with Prompts and Instructions](#-composing-agents-with-prompts-and-instructions)
- [ðŸ’¡ Decision Framework: When to Use Each File Type](#-decision-framework-when-to-use-each-file-type)
- [ðŸ§ª Testing and Validating Agents](#-testing-and-validating-agents)
- [ðŸŽ¯ Conclusion](#-conclusion)
- [ðŸ“š References](#-references)
- [ðŸ“Ž Appendix: Unified Agent Architecture (v1.107+)](#-appendix-unified-agent-architecture-v1107)
...
For comprehensive coverage of Agent HQ, session management, and work tree isolation workflows, see [ðŸ“Ž Appendix: Unified Agent Architecture](#-appendix-unified-agent-architecture-v1107).
...
| **Tool Control** | âœ… Full control via `tools` field | âŒ No tool configuration |
| **Handoffs** | âœ… Workflow transitions | âŒ Not supported |
...
## ï¿½ðŸ”„ How Agents Differ from Prompts and Instructions
...
> **ðŸ“˜ Note**: For detailed guidance on Agent Skills, see [How to Structure Content for GitHub Copilot Skills](./06.00-how_to_structure_content_for_copilot_skills.md).
...
## ðŸ”— Agent Interactions and Handoffs
...
|---|---|---|---|---|
| `true` (default) | `false` (default) | âœ… Yes | âœ… Yes | Normal agent |
| `false` | `false` | âŒ No | âœ… Yes | Background worker subagent |
| `true` | `true` | âœ… Yes | âŒ No | User-only agent (no auto-delegation) |
| `false` | `true` | âŒ No | âŒ No | Manually invoked subagent only |

> âš ï¸ **Deprecation:** The `infer` property previously combined both behaviors. `infer: true` was equivalent to `user-invokable: true` + `disable-model-invocation: false`. `infer: false` was equivalent to `user-invokable: false` + `disable-model-invocation: true`. Migrate to the new properties for independent control.
...
> ðŸ“ **Note:** When using the `agents` property, ensure `runSubagent` is included in your `tools` list.
...
Restrict tools to match each agent's roleâ€”read-only agents shouldn't have `editor`, security-sensitive agents shouldn't have `fetch` or `web_search`. For detailed selection strategies and safety patterns, see the [tool selection strategy](./09.50-how_to_leverage_tools_in_prompt_orchestrations.md#-tool-selection-strategy) and [tool restrictions](./09.50-how_to_leverage_tools_in_prompt_orchestrations.md#ï¸-tool-restrictions-and-safety) sections.

## ðŸ§© Composing Agents with Prompts and Instructions
...
## ðŸ’¡ Decision Framework: When to Use Each File Type
...
> **ðŸ“˜ For detailed Skills guidance**: See [How to Structure Content for GitHub Copilot Skills](./06.00-how_to_structure_content_for_copilot_skills.md)
...
- âŒ Mixed responsibilities (review + generate + test)
- âŒ Excessive background information
- âŒ Detailed explanations better suited for instruction files
- âŒ Too many tools (should be focused)
- âŒ No clear handoff strategy
...
| > 2,000 tokens | 3+ seconds | âš ï¸ Very High | ðŸ”´ Refactor required |
...
## ðŸ§ª Testing and Validating Agents
...
**âŒ Bad example (automatic send when user should review):**
```yaml
---
handoffs:
...
**âŒ Bad example (manual send when automatic makes sense):**
```yaml
---
handoffs:
  - label: "Format Code"
...
**âŒ Bad example (cloud agent without target specification):**
```yaml
---
...
- **[Custom Agents in VS Code](https://code.visualstudio.com/docs/copilot/customization/custom-agents)** `[ðŸ“˜ Official]`  
...
- **[Use Prompt Files in VS Code](https://code.visualstudio.com/docs/copilot/customization/prompt-files)** `[ðŸ“˜ Official]`  
...
- **[Use Custom Instructions in VS Code](https://code.visualstudio.com/docs/copilot/customization/custom-instructions)** `[ðŸ“˜ Official]`  
...
- **[Use Tools in Chat](https://code.visualstudio.com/docs/copilot/chat/chat-tools)** `[ðŸ“˜ Official]`  
...
- **[VS Code Background Agents Documentation](https://code.visualstudio.com/docs/copilot/agents/background-agents)** `[ðŸ“˜ Official]`  
...
- **[VS Code Agents Tutorial](https://code.visualstudio.com/docs/copilot/agents/agents-tutorial)** `[ðŸ“˜ Official]`  
...
- **[The GitHub Copilot customization stack](../01-overview/01.00-the_github_copilot_customization_stack.md)**  
  *Series overview mapping every customization mechanismâ€”prompts, instructions, agents, skills, hooks, MCP, models, and more. Start here to understand how all the pieces fit together.*
...
- **[Awesome GitHub Copilot - Custom Agents Examples](https://github.com/github/awesome-copilot/tree/main)** `[ðŸ“’ Community]`  
...
## ðŸ“Ž Appendix: Unified Agent Architecture (v1.107+)
