...
- [🏗️ Agent File Structure](#-agent-file-structure)
...
> **📝 Note**: For detailed guidance on Agent Skills, see [How to Structure Content for GitHub Copilot Skills](./06.00-how_to_structure_content_for_copilot_skills.md).
...
model: Preferred model (e.g., Claude Sonnet 4.6, GPT-5 mini)
...
| `model` | String | No | Preferred AI model (e.g., "Claude Sonnet 4.6", "GPT-5 mini"). BYOK models supported in v1.107+ |
...
> ⚠️ **Deprecation:** The `infer` property previously combined both behaviors. `infer: true` was equivalent to `user-invokable: true` + `disable-model-invocation: false`. `infer: false` was equivalent to `user-invokable: false` + `disable-model-invocation: true`. Migrate to the new properties for independent control.
...
For complete coverage of subagent orchestration patterns (coordinator/worker, multi-perspective review, parallel execution), see [How to Design Subagent Orchestrations](./11.00-how_to_design_subagent_orchestrations.md).

## 🔧 Tool configuration for agents

> **Comprehensive tool documentation has been consolidated into a dedicated article.** For the complete tool architecture—YAML capability declarations, runtime tool catalog, token costs, selection strategies by agent role, and practical patterns—see [How to Leverage Tools in Prompt Orchestrations](./09.50-how_to_leverage_tools_in_prompt_orchestrations.md).
...
Restrict tools to match each agent's role—read-only agents shouldn't have `editor`, security-sensitive agents shouldn't have `fetch` or `web_search`. For detailed selection strategies and safety patterns, see the [tool selection strategy](./09.50-how_to_leverage_tools_in_prompt_orchestrations.md#-tool-selection-strategy) and [tool restrictions](./09.50-how_to_leverage_tools_in_prompt_orchestrations.md#%EF%B8%8F-tool-restrictions-and-safety) sections.
...
> **📝 For detailed Skills guidance**: See [How to Structure Content for GitHub Copilot Skills](./06.00-how_to_structure_content_for_copilot_skills.md)
...
- **[The GitHub Copilot customization stack](../01-overview/01.00-the_github_copilot_customization_stack.md)**  
...
- **[How to Name and Organize Prompt Files](../02-getting-started/02.00-how_to_name_and_organize_prompt_files.md)**  
