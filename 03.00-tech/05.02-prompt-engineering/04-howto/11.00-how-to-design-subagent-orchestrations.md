...
description: "Learn how to use VS Code's subagent capabilities—the runSubagent tool, context isolation, the agents property, and orchestration patterns—to delegate focused subtasks within multi-agent workflows."
---
...
Subagents are the execution mechanism that makes orchestration practical. Where [article 10](./10.00-how-to-design-orchestrator-prompts.md) teaches you how to *design* orchestrators—decomposing tasks into phases, defining specialist roles, planning execution flow—this article teaches you how to *implement* the delegation itself using VS Code's subagent capabilities.
...
- [Understanding subagent execution 🎯](#understanding-subagent-execution)
- [The `runSubagent` tool 📋](#the-runsubagent-tool)
- [Custom agents as subagents 🔧](#custom-agents-as-subagents)
- [Controlling agent visibility and invocation 🔒](#controlling-agent-visibility-and-invocation)
- [Restricting subagent access with `agents` 🎯](#restricting-subagent-access-with-agents)
- [Orchestration patterns 🔀](#orchestration-patterns)
- [Parallel execution ⚡](#parallel-execution)
- [Token implications 💰](#token-implications)
...
| Aspect | Behavior |
|--------|----------|
| **<mark>Context isolation</mark>** | Each subagent starts with a clean context window—no inherited conversation history |
...
| **<mark>Result format</mark>** | Only the subagent's final result returns to the main agent—not intermediate tool calls |
...
|--------|----------|-------------|
| **<mark>Relationship to parent</mark>** | Child—reports results back | Independent—no connection |
...
|:---:|:---:|---|
| <mark>`true`</mark> | `false` | **Full access** — User can select it AND agents can auto-invoke it |
| <mark>`true`</mark> | <mark>`true`</mark> | **User-only** — Visible in dropdown but protected from auto-invocation |
| <mark>`false`</mark> | <mark>`false`</mark> | **Subagent-only** — Hidden from dropdown but available for orchestration |
| <mark>`false`</mark> | <mark>`true`</mark> | **Locked** — Neither visible nor auto-invokable (must be explicitly allowed via `agents` array) |
...
The main agent processes condensed summaries instead of accumulating raw data. For detailed token budgeting strategies, see [How to Optimize Token Consumption During Prompt Orchestrations](./13.00-how-to-optimize-token-consumption-during-prompt-orchestrations.md).
...
- **<mark>Context isolation</mark> is the primary benefit** — Subagents prevent context bloat by processing details in their own window and returning only summaries
...
- **Control visibility with two properties** — <mark>`user-invokable`</mark> controls the dropdown, <mark>`disable-model-invocation`</mark> controls auto-invocation; combine them for four distinct access patterns
- **<mark>Restrict with the `agents` array</mark>** — Explicitly list allowed subagents in your orchestrator to prevent unintended delegation
- **<mark>Parallel execution</mark> accelerates independent tasks** — VS Code can spawn concurrent subagents for truly independent work
...
- **[How to Manage Information Flow](./12.00-how-to-manage-information-flow-during-prompt-orchestrations.md)** — Data contracts, communication pathways, and context window dynamics across the customization stack
- **[How to Optimize Token Consumption](./13.00-how-to-optimize-token-consumption-during-prompt-orchestrations.md)** — Token budgeting, context compression, and cost management for multi-agent workflows
...
**[VS Code: Agent Tools](https://code.visualstudio.com/docs/copilot/agents/agent-tools)** `[📘 Official]`  
...
**[How to Design Orchestrator Prompts for Multi-Agent Workflows](./10.00-how-to-design-orchestrator-prompts.md)** `[📙 Internal]`  
...
Detailed guide for individual agent file design—frontmatter properties, tool selection, instruction writing. A prerequisite for designing the specialist agents used in subagent orchestrations.
...
The data side of orchestration—how information flows between agents, context window dynamics, communication pathways, and structured handoff contracts.
...
Token management strategies that directly complement subagent architecture—context isolation reduces cost, but you need structured approaches to maximize the savings.
...
article_metadata:
  filename: "11.00-how-to-design-subagent-orchestrations.md"
  created: "2026-02-22"
  last_updated: "2026-02-22"
  version: "1.0"
  status: "draft"
  changelog: "11.00-how_to_design_subagent_orchestrations.changelog.md"
  word_count: 3400
...
cross_references:
  ...
  position: 11
  previous: "10.00-how-to-design-orchestrator-prompts.md"
  next: "12.00-how-to-manage-information-flow-during-prompt-orchestrations.md"
  prerequisites:
