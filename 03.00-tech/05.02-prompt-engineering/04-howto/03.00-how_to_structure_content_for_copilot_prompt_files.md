...
- [âœï¸ Compose Clear and Structured Content](#ï¸-compose-clear-and-structured-content)
- [ðŸŽ¨ Advanced Prompt Structuring Patterns](#-advanced-prompt-structuring-patterns)
- [ðŸ“ Organize Supporting Materials](#-organize-supporting-materials)
- [âš™ï¸ Environment-Specific Considerations](#ï¸-environment-specific-considerations)
- [ðŸ§ª Testing and Validating Prompts](#-testing-and-validating-prompts)
...
## âœï¸ Compose Clear and Structured Content
...
```
my-app/                      â† Generated project root
â”œâ”€â”€ .env.template           â† Committed to repo (no secrets)
â”œâ”€â”€ .env                    â† Local only (.gitignored, contains secrets)
â”œâ”€â”€ src/
â”‚   â””â”€â”€ app.py
...
```
.github/prompts/
â”œâ”€â”€ scaffold-app.prompt.md   â† Prompt that GENERATES .env.template
â””â”€â”€ (no .env.template here)  â† Templates are outputs, not inputs
```

For **reusable templates** referenced by multiple prompts, use `.github/prompt-snippets/` for Markdown fragments or `.github/templates/` for file templates, but these are rareâ€”most prompts generate templates dynamically based on user requirements.
...
**Platform Support:**
- âœ… **VS Code**: Full support for all chat variables
- âš ï¸ **Visual Studio**: Limited support; check version documentation
...
|---------|----------------|----------------------|
| **Chat Variables** | âœ… Full (`${workspaceFolder}`, `${file}`, etc.) | âš ï¸ Limited support |
| **File References** | âœ… `#file:path` syntax | âš ï¸ Check version docs |
| **Tools** | âœ… Full (`#codebase`, `#fetch`, `#web`) | âš ï¸ Partial support |
| **MCP Servers** | âœ… Preview (1.106+) | âŒ Not supported |
| **Tool Control** | âœ… `tools` YAML field | âš ï¸ Limited |
...
**For comprehensive tool documentation**, see [How to Leverage Tools in Prompt Orchestrations](./09.50-how_to_leverage_tools_in_prompt_orchestrations.md) which provides the complete tool architecture, runtime tool catalog, token costs, selection strategies, and practical patterns.

## ðŸŽ¨ Advanced Prompt Structuring Patterns
...
- **ðŸ” Discovery**: Use case ideation, evaluation, research, resource gathering
- **âš¡ Rapid Prototyping**: Requirements definition, data generation, code scaffolding, code generation
- **ðŸšš Delivery**: Architecture design, deployment planning, webinar content
- **ðŸ’» GitHub Copilot**: Repository-specific prompts for in-IDE workflows
...
.github/prompts/
â”œâ”€â”€ discovery/
â”‚   â”œâ”€â”€ use-case-ideation.prompt.md
â”‚   â””â”€â”€ requirements-gathering.prompt.md
â”œâ”€â”€ development/
â”‚   â”œâ”€â”€ code-scaffolding.prompt.md
â”‚   â””â”€â”€ code-generation.prompt.md
â”œâ”€â”€ quality/
â”‚   â”œâ”€â”€ grammar-review.prompt.md
â”‚   â””â”€â”€ security-review.prompt.md
â””â”€â”€ documentation/
    â”œâ”€â”€ article-writing.prompt.md
    â””â”€â”€ api-docs-generation.prompt.md
```
...
## ðŸ“ Organize Supporting Materials
...
- <mark>**Project documentation**</mark>: Use a `.copilot/context/` (optional) folder to store rich informationâ€”API contracts, data schemas, domain terms, architecture decisions and diagramsâ€”which <mark>the Copilot engine can search</mark>. VS Code and Visual Studio both index these files to improve the relevance of suggestions.
...
```
.github/instructions/
â”œâ”€â”€ csharp-backend.instructions.md     (applyTo: "src/backend/**/*.cs")
â”œâ”€â”€ typescript-frontend.instructions.md (applyTo: "src/frontend/**/*.ts")
â”œâ”€â”€ python-ml.instructions.md          (applyTo: "ml/**/*.py")
â””â”€â”€ sql-migrations.instructions.md     (applyTo: "db/migrations/**/*.sql")
```
...
## ðŸ“ Content Length Best Practices
...
**What Are Tokens?**
- Tokens are the basic units that language models process (roughly 1 token â‰ˆ 4 characters or 0.75 words)
...
| 2,500-4,000 tokens | 15-25 seconds | 60-75% | âš ï¸ Use sparingly |
| > 4,000 tokens | 25+ seconds | < 60% | ðŸ”´ Refactor required |
...
# ðŸ§ª Testing and Validating Prompts
...
1. **Peer test** â€” Have another team member run the prompt without guidance
2. **Document** â€” Add usage examples to your team's prompt catalog
3. **Changelog** â€” Note significant changes in commit messages
4. **Monitor** â€” Collect feedback after rollout, iterate as needed

# ðŸŽ¯ Conclusion

<mark>Effective prompt-file design combines a well-crafted YAML header with a clear, structured body and often includes templates or examples</mark>. For sophisticated, reusable prompts, consider adopting the **three-part architecture** (System Message, User Prompt Template, Example Usage) demonstrated by Microsoft's AI Prompt Bookâ€”this pattern separates concerns, maximizes reusability, and provides clear guidance for both AI models and human users. By respecting the official file locations (e.g., `.github/prompts/` for prompt files), organizing prompts by workflow categories, and understanding the differences between VS Code and Visual Studio capabilities, you can create prompt libraries that provide consistent and high-quality results across your development environments.
...
**âœ… Solution: Proper YAML structure:**
```yaml
---
name: create-component
...
**âœ… Solution: Specific, measurable criteria:**
```markdown
---
...
**âœ… Solution: Focus on the happy path, reference instructions for standards:**
```markdown
---
name: create-api-endpoint
tools: [fetch]
...
**âœ… Solution: Provide concrete examples:**
```markdown
---
name: generate-test
---
...
**âœ… Solution: Use valid agents and tools:**
```yaml
---
name: refactor-code
...
**âœ… Solution: Use variables and pattern matching:**
```markdown
---
name: update-config
---
...
**âœ… Solution: Use variable substitution (VS Code 1.99+):**
```yaml
---
name: create-react-component
description: "Generate React component with TypeScript"
...
**Usage:** `/create-react-component Button` â†’ substitutes `${componentName}` with `Button`
...
**âœ… Solution: Cross-IDE compatible prompt:**
```yaml
---
name: analyze-codebase
...
- **[GitHub Copilot Prompt Engineering Guide](https://docs.github.com/en/copilot/using-github-copilot/prompt-engineering-for-github-copilot)** `[ðŸ“˜ Official]`  
...
- **[Customize Chat Responses and Set Context (Visual Studio)](https://learn.microsoft.com/en-us/visualstudio/ide/copilot-chat-context?view=visualstudio)** `[ðŸ“˜ Official]`  
...
- **[GitHub Copilot in Visual Studio](https://learn.microsoft.com/en-us/visualstudio/ide/visual-studio-github-copilot-extension)** `[ðŸ“˜ Official]`  
...
- **[OpenAI Prompt Engineering Guide](https://platform.openai.com/docs/guides/prompt-engineering)** `[ðŸ“˜ Official]`  
...
- **[Anthropic's Prompt Engineering Tutorial](https://docs.claude.com/en/docs/prompt-engineering)** `[ðŸ“˜ Official]`  
  *This tutorial offers insights into how large language models interpret instructions, including the importance of clear role definition, structured formatting, and providing contextâ€”all concepts that directly support the best practices outlined in this article for creating effective prompt files.*
...
- **[VS Code MCP Servers Documentation](https://code.visualstudio.com/docs/copilot/customization/mcp-servers)** `[ðŸ“˜ Official]`  
...
- **[Microsoft AI Prompt Book for Architects](https://github.com/microsoft-partner-solutions-ai/ai-prompt-book)** `[ðŸ“˜ Official]`  
...
- **[GitHub Awesome Copilot Repository](https://github.com/github/awesome-copilot)** `[ðŸ“˜ Official]`  
...
- **Example**: `name: react-form` â†’ Use `/react-form` in VS Code
...
- âœ… `name` (or rely on filename)
- âœ… `description` (strongly recommended)
...
**Common Mistakes:**
- âŒ Using spaces in `name` (use hyphens: `code-review` not `code review`)
- âŒ Overly long `description` (keep under 100 chars)
- âŒ Specifying unavailable models (check user's subscription)
- âŒ Over-restricting tools (limits functionality unnecessarily)
- âŒ Vague argument hints (be specific about expected format)
...
# Appendix B: Tools reference (moved)

> **This appendix has been consolidated into a dedicated article.** For the complete tool architectureâ€”YAML capability declarations, runtime tool catalog, MCP tools, token costs, selection strategies, and practical patternsâ€”see [How to Leverage Tools in Prompt Orchestrations](./09.50-how_to_leverage_tools_in_prompt_orchestrations.md).

**Quick reference â€” core YAML capabilities:**

| Capability | Purpose | Read-Only |
|------------|---------|:---------:|
| `codebase` | Semantic code search | Yes |
| `editor` | File read/write operations | No |
| `filesystem` | Directory navigation, file queries | Yes |
| `fetch` | Retrieve web content | Yes |
| `web_search` | Internet search | Yes |
| `search` | Workspace text search | Yes |
| `usages` | Find code usages/references | Yes |
| `problems` | Get errors/warnings | Yes |
| `changes` | View git changes | Yes |
