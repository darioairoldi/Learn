...
A <mark>"good generic prompt" doesn't exist</mark> — there exists only a good prompt *for that specific model*. Different models have fundamentally different behaviors, strengths, and optimal prompting strategies. What works brilliantly with Claude may fail with GPT-4o; what excels with Gemini may confuse reasoning models.
...
- [The compiler analogy: why models matter 🎯](#the-compiler-analogy-why-models-matter)
- [Model families and their characteristics 📊](#model-families-and-their-characteristics)
- [Standard models vs. reasoning models 🧠](#standard-models-vs-reasoning-models)
- [Model-specific prompting strategies 🔧](#model-specific-prompting-strategies)
- [Multi-model architecture patterns 🏗️](#multi-model-architecture-patterns)
- [BYOK: bring-your-own-key providers 🔑](#byok-bring-your-own-key-providers)
- [Model selection decision framework 📋](#model-selection-decision-framework)
- [Key considerations ⚠️](#key-considerations)
- [Foundry reasoning-model operations 🏭](#foundry-reasoning-model-operations)
...
Claude models excel with <mark>clarity and context</mark> — clear XML-tagged structure, explicit context about your norms and preferences, and well-organized reference material. Think of Claude as a brilliant but new colleague who needs explicit context about your expectations.
...
The most important conceptual division isn't between providers — it's between <mark>standard models</mark> and <mark>reasoning models</mark>.
...
## 🏭 Foundry reasoning-model operations

When you use reasoning models through Azure AI Foundry, model selection also includes provider-specific operating rules.

### MAI-Thinking-1 operational profile (current preview)

- Preview model, with preview terms and no production SLA by default.
- 256K context window with output capped at 64K tokens.
- Function tool calling supported.
- Global Standard deployment supported; PTU not currently supported.

### Reasoning-state and API semantics

- Preserve returned reasoning-state envelopes exactly when required by the provider (for MAI-Thinking-1: encrypted reasoning state).
- Use provider-specific parameter names (`max_completion_tokens`, not `max_tokens`).
- Budget input, output, and reasoning tokens together in the same context window.
- For streaming flows, handle safety interruptions and partial outputs as first-class operational scenarios.

### Foundry model-selection loop

In Foundry environments, treat model selection as a measurable loop:

1. Compare candidates with model benchmarks.
2. Validate quality and safety on your own evaluation dataset.
3. Monitor production telemetry for quality, latency, safety, and cost.
4. Promote, rollback, or reroute based on thresholds.

This extends the general re-validation rule into a concrete operations pattern. Use the conceptual strategy in this article together with the practical playbook in [How to optimize prompts for specific models](../04-howto/08.00-how-to-optimize-prompts-for-specific-models.md) and the cost-to-outcome framing in [Optimizing AI systems on the cost-to-outcome frontier](../05-analysis/23-optimizing-ai-systems-on-the-cost-to-outcome-frontier.md).
...
- [How to optimize prompts for specific models](../04-howto/08.00-how-to-optimize-prompts-for-specific-models.md) — detailed, actionable optimization techniques per model family
- [How Copilot assembles and processes prompts](./01.02-how-copilot-assembles-and-processes-prompts.md) — understanding context windows and attention patterns
- [Chat modes, Agent HQ, and execution contexts](./01.08-chat-modes-agent-hq-and-execution-contexts.md) — how the `model` field interacts with execution modes
...
**[VS Code Copilot Language Models Documentation](https://code.visualstudio.com/docs/copilot/language-models)** `[📘 Official]`
...
Official MAI-Thinking-1 deployment and API guidance, including preview constraints, token budgets, reasoning-state handling, and tool-calling behavior.

**[Observability in generative AI](https://learn.microsoft.com/azure/foundry/concepts/observability)** `[📘 Official]`
Foundry observability model across evaluation, monitoring, and tracing. Useful for turning model selection into a measurable lifecycle loop.
...
Foundry benchmark and side-by-side comparison workflow for quality, safety, cost, and throughput trade-offs.
...
article_metadata:
  filename: "01.07-understanding-llm-models-and-model-selection.md"
  created: "2026-03-01"
