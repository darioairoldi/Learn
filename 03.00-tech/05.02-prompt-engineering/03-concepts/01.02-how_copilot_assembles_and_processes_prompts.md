...
Layer 6 is where <mark>custom agents</mark> inject their persona and workflow. This only happens when a custom agent is active (selected in the agent picker). The agent body acts as a full identity override — it doesn't just provide information, it tells the model *who it is* and *how it should behave*.
