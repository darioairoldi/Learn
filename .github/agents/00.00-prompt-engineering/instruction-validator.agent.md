...
|---|---|---|---|
| 1 | **description** | Present, non-empty, one sentence | CRITICAL |
| 2 | **applyTo** | Present, valid glob pattern | CRITICAL |
| 3 | **applyTo specificity** | Pattern matches intended files — not overly broad | HIGH |

### Structure Checks

| # | Check | Criteria | Severity |
|---|---|---|---|
| 4 | **Title** | H1 heading present | HIGH |
| 5 | **Purpose section** | Brief description of what rules enforce | HIGH |
| 6 | **Rules sections** | At least one section with actionable rules | CRITICAL |
| 7 | **Flat structure** | File is in `.github/instructions/` root (no subfolders) | MEDIUM |

### Content Quality Checks

| # | Check | Criteria | Severity |
|---|---|---|---|
| 8 | **Imperative language** | Uses MUST, WILL, NEVER — not suggestions | HIGH |
| 9 | **Conciseness** | Rules are direct and scannable — lists and tables preferred | MEDIUM |
| 10 | **No embedded knowledge** | Content >10 lines references context files instead of embedding | HIGH |
| 11 | **Context file references** | Uses `📖` references to context files for detailed guidance | MEDIUM |

### Integrity Checks

| # | Check | Criteria | Severity |
|---|---|---|---|
| 12 | **Token budget** | File =1,500 tokens (~120 lines) | CRITICAL |
| 13 | **No applyTo overlap** | Pattern doesn't overlap with other instruction files' patterns | CRITICAL |
| 14 | **No rule contradictions** | Rules don't conflict with context files or other instructions | CRITICAL |
| 15 | **References resolve** | All `📖` links point to existing context files | HIGH |
| 16 | **No rule duplication** | Rules not duplicated from another instruction or context file | HIGH |
| 17 | **context_dependencies completeness** | `context_dependencies` lists ALL context folders referenced via `📖` in the file | HIGH |
