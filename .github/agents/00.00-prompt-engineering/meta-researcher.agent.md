...
3. **Scan the PE artifact structure** by listing all 9 artifact locations to identify structural coverage:
   ...
   - `.github/agents/00.00-prompt-engineering/` — agent files
   - `.github/prompts/00.00-prompt-engineering/` — prompt files
...
6. **Load authoritative sources list**: Read the "Authoritative Sources (curated)" section from `.copilot/context/00.00-prompt-engineering/05.04-meta-review-log.md`. Prioritize these sources over general internet search in Step 2's internet research phase — consult curated sources first, then supplement with general search only when curated sources don't cover the topic. After consulting each source, update its "Last checked" date in the table.

7. **Internet research** (unless `--no-external`): Use `fetch_webpage` to research authoritative sources — official documentation, release notes, specifications, research papers, established community best practices. Prioritize primary sources (vendor docs, official specs) over secondary sources (blog posts, tutorials).
