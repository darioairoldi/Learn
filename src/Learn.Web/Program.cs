...
public class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        using (var activity = Observability.ActivitySource.StartMethodActivity(logger, new { args }))
        {
            ...
            // configuration (Testmc → samplestmcstitn01) is applied correctly.
            //
            // Authentication reuses Diginsight's DefaultCredentialProvider — the same chain the
            // Key Vault integration uses. In Development it chains Azure CLI → VS Code → Visual
    ...
    /// <summary>
    /// Maps a request path to a blob name: "" → index.html, "foo/" or extensionless "foo" →
