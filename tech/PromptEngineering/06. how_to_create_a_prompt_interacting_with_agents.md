...
- [🧠 Intelligent Architecture Analysis (Phase 3)](#-intelligent-architecture-analysis-phase-3)
...
Phase 3: Architecture Analysis (Orchestrator)
    ...
    │
    ▼
Phase 4/4a/4b: Build (→ prompt-builder or agent-builder)
    │
    │ Builder creates file(s) following research
    │ Orchestrator presents results to user
    │
    ▼
Phase 5: Validation (→ prompt-validator)
    │
    │ Validator checks quality, reports issues
    │ Automatic handoff (send: true)
    │
    ▼
Phase 6: Fix (Optional → prompt-updater)
    │
    │ If issues found: updater applies fixes
...
## 🧠 Intelligent Architecture Analysis (Phase 3)
...
handoffs:
  ...
  - label: "Validate agent quality"
    ...
    send: true
argument-hint: 'Describe the prompt: ${input:purpose:What is the prompt purpose?}'
---
```
...
7. **Context Optimization**: Lazy loading, context caching, minimal orchestrator tools
...
Orchestrator (coordinates, never implements)
    ...
    ├── Builder (write, creates new files)
    ├── Validator (read-only, checks quality)
...
**[Context Engineering Principles](./.copilot/context/prompt-engineering/context-engineering-principles.md)** `[📒 Community]`  
...
**[Tool Composition Guide](./.copilot/context/prompt-engineering/tool-composition-guide.md)** `[📒 Community]`  
...
validations:
  ...
  consistency_review:
    ...
    gaps_identified: 8
    gaps_addressed: 8
    changes_summary: "Added MCP integration section, context engineering optimization section, enhanced tool configuration details, updated references with classifications and 8 new sources"
---
-->
