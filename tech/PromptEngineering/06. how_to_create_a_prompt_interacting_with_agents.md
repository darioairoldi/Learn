...
Custom agents aren't limited to interactive chat sessions. VS Code provides multiple deployment contexts for your orchestrated workflows:

#### Background Agents

<mark>**Background agents**</mark> run custom agents autonomously without blocking your workflow. Once configured, a background agent executes tasks in the background while you continue working.

**Use cases for orchestrations:**
- Long-running validation workflows that check entire codebases
- Automated documentation generation triggered by file changes
- Scheduled code quality audits using your specialized agents

#### Cloud Agents

<mark>**Cloud agents**</mark> execute your custom agents in GitHub's cloud infrastructure, enabling autonomous task completion without keeping VS Code open.

**Benefits for multi-agent workflows:**
- Offload complex orchestrations to cloud compute
- Continue agent workflows across sessions
- Scale resource-intensive validation tasks

Both background and cloud agents can reuse the same `.agent.md` files you create for interactive use, making your orchestrator patterns portable across execution contexts.
