...
When a handoff occurs (via `send: true` or user click), the **entire conversation history** flows to the target agent. Use natural language references to prior outputs:
...
```markdown
✅ **Efficient**: 
"For validation rules, see .copilot/context/prompt-engineering/validation-patterns.md"

❌ **Wasteful**:
[Embeds 200 lines of validation rules in handoff prompt]
```

#### Pattern 2: Selective Context Passing

When handing off to validator, pass:
- ✅ File path to validate
- ✅ Specification from Phase 3
- ✅ Critical boundaries to check

**DO NOT pass**:
- ❌ Entire research report (validator doesn't need pattern history)
- ❌ User's original request (already incorporated in spec)
- ❌ Template discovery process (not relevant to validation)

#### Pattern 3: Progressive Summarization

Each phase produces **preprocessed summaries** for downstream consumption:

```markdown
Phase 2: Research → **3,000 token research output**
        ↓ (compress to essentials for next phase)
Phase 3: Structure Definition → **500 token pattern summary**
        ↓ (specification only)
Phase 4: Builder → Receives **500 tokens**, not 3,000
```
...
1. grep_search for specific pattern (500 tokens)
2. read_file only matched files (2,000 tokens)
3. semantic_search only if grep finds nothing (2,000 tokens)

❌ **Expensive Search Order**:
1. semantic_search for broad query (2,000 tokens)
  ...
  - label: "{Action Description}"
    ...
    prompt: |
      ...
      **Context from Previous Phase** (reference, not embed):
      ...
      - Specification: {link to Phase 3 output}
