...
- <mark>**`.github/prompts/`**</mark> (workspace prompts). This is the default folder for <mark>**workspace-level prompt files**</mark> and is recognized automatically by both VS Code and Visual Studio (17.10+). Any `.prompt.md` file here becomes available in Copilot Chat: <mark>in VS Code it appears as `/promptName`, and in Visual Studio it appears as `#promptName`</mark>.
- **User prompt files** (VS Code only). <mark>User-scope prompts</mark> are stored in a folder within your VS Code profile (e.g., `%APPDATA%\Code\User\prompts\` on Windows, `~/Library/Application Support/Code/User/prompts/` on macOS, or `~/.config/Code/User/prompts/` on Linux). These prompts are available across all your VS Code workspaces. They do <mark>**not synchronise to your team via Git**</mark>, but you can enable Settings Sync to share them across your own machines. Visual Studio currently has no equivalent mechanism; it only consumes repository-level prompt files.

### Location **not** automatically supported

Some community blogs suggest grouping Copilot files under a `.github/copilot/prompts/` folder. <mark>**Officially, this location is not recognised by default**</mark>. Both VS Code and Visual Studio expect prompt files in `.github/prompts/` unless you change VS Code's `chat.promptFilesLocations` setting. Visual Studio does not provide a configurable prompt-file location, so it cannot automatically read from `.github/copilot/prompts/`.
