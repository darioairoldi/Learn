...
- [📎 Appendix B: Bring-Your-Own-Key Model Management](#-appendix-b-bring-your-own-key-model-management)
- [📎 Appendix C: Legacy .chatmode.md Migration](#-appendix-c-legacy-chatmodemd-migration)
- [📎 Appendix D: JetBrains IDE Support](#-appendix-d-jetbrains-ide-support)
- [📎 Appendix E: Deprecated VS Code Settings](#-appendix-e-deprecated-vs-code-settings)
...
# 📎 Appendix C: Legacy .chatmode.md Migration
...
# 📎 Appendix D: JetBrains IDE Support
...
# 📎 Appendix E: Deprecated VS Code Settings
...
# 📎 Appendix B: Bring-Your-Own-Key Model Management
...
# 📎 Appendix F: GitHub Copilot vs Alternative AI Coding Tools

As AI coding assistants proliferate, teams often ask how GitHub Copilot compares to alternatives like Cursor, Cline, Continue Dev, and Windsurf. Understanding these differences helps teams choose the right tool—or use them together strategically.
...
├── prompts/
│   └── *.prompt.md             # Reusable prompts (slash commands)
├── agents/
│   └── *.agent.md              # Custom agents with personas
└── instructions/
    └── *.instructions.md       # Path-specific rules
```

**Key characteristics:**
- ✅ **Multi-file approach** - Separate concerns cleanly
- ✅ **YAML frontmatter** - Rich metadata (tools, model, handoffs)
- ✅ **Glob pattern matching** - Instructions auto-apply based on file patterns
- ✅ **Cross-IDE support** - Works in VS Code, Visual Studio, JetBrains (with variations)
- ✅ **Handoff support** - Agents can transfer control to other agents
...
**Key characteristics:**
- ✅ **Simple single-file config** - All rules in one place
- ✅ **Composer mode** - Multi-file editing with planning
- ✅ **@docs reference** - Index documentation folders for context
- ✅ **Apply mode** - Incremental edits with verification
- ❌ **No multi-agent orchestration** - Single context model
- ❌ **Proprietary format** - Not compatible with other tools
- ❌ **Requires Cursor IDE** - Standalone application, not a plugin
...
- ✅ **Task-oriented** - Describe tasks in natural language
- ✅ **Tool-use focused** - Can execute commands, edit files, browse web
- ✅ **API key flexibility** - Bring your own Anthropic/OpenAI key
- ❌ **No repository-level configuration** - Must specify context each time
- ❌ **No reusable prompt library** - No equivalent to `.prompt.md` files
- ❌ **Limited IDE integration** - VS Code only
...
```
.continue/
└── config.json           # Model configuration, slash commands, context providers
```

**Key characteristics:**
- ✅ **Open source** - Apache 2.0 license
- ✅ **Model agnostic** - Supports 20+ LLM providers
- ✅ **Custom slash commands** - Define your own commands in JSON
- ✅ **Context providers** - Integrate with GitHub, GitLab, Jira, etc.
- ✅ **Autocomplete + chat** - Both inline and chat interfaces
- ❌ **JSON configuration** - Less readable than Markdown
- ❌ **No multi-agent support** - Single context model
- ❌ **Limited instruction flexibility** - No glob-based pattern matching
...
**Key characteristics:**
- ✅ **Flows feature** - Multi-step task automation
- ✅ **Cascade mode** - Context-aware multi-file editing
- ✅ **Simple configuration** - Single rules file
- ❌ **Proprietary format** - Not compatible with other tools
- ❌ **Requires Windsurf IDE** - Standalone application
- ❌ **Newer tool** - Smaller community, fewer established patterns
...
| **Inline completion** | ✅ | ✅ | ❌ | ✅ | ✅ |
| **Chat interface** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Multi-file editing** | ⚠️ Limited | ✅ Composer | ⚠️ Manual | ⚠️ Limited | ✅ Cascade |
| **Custom instructions** | ✅ Multiple files | ✅ Single file | ❌ | ⚠️ JSON only | ✅ Single file |
| **Reusable prompts** | ✅ `.prompt.md` | ⚠️ Via composer | ❌ | ✅ Slash commands | ❌ |
| **Multi-agent workflow** | ✅ Handoffs | ❌ | ❌ | ❌ | ❌ |
| **Pattern-based rules** | ✅ Glob patterns | ❌ | ❌ | ❌ | ❌ |
| **Model choice** | ⚠️ Limited | ✅ Multiple | ✅ BYOK | ✅ 20+ providers | ⚠️ Limited |
| **Open source** | ❌ | ❌ | ✅ | ✅ | ❌ |
| **Cross-IDE support** | ✅ 3+ IDEs | ❌ Cursor only | ❌ VS Code only | ❌ VS Code only | ❌ Windsurf only |
| **Enterprise features** | ✅ | ✅ | ❌ | ⚠️ Self-hosted | ✅ |
...
**What transfers:**
- General coding guidelines from `.cursorrules` → `.github/copilot-instructions.md`
- Documentation references → `.copilot/context/` folder

**What requires recreation:**
- Composer-specific workflows → Custom agents with handoffs
- `@docs` indexed folders → Explicit references in instructions
...
**What transfers:**
- Slash command prompts → `.prompt.md` files in `.github/prompts/`
- General guidelines → `.github/copilot-instructions.md`

**What requires recreation:**
- Context providers (GitHub, GitLab) → Use native Copilot integrations
- Custom model configurations → Use Copilot's model picker or BYOK
...
- `.copilot/context/` documentation → `docs/` folder (for `@docs` indexing)

**What requires recreation:**
- Individual `.prompt.md` files → Manual prompts in Composer
- `.agent.md` files with handoffs → Multi-step manual workflows
- Path-specific `.instructions.md` → Consolidated in single `.cursorrules`
...
- ✅ Enterprise organizations with GitHub Enterprise subscriptions
- ✅ Teams using multiple IDEs (VS Code + Visual Studio + JetBrains)
- ✅ Projects requiring granular, path-specific coding standards
- ✅ Complex workflows needing multiple specialized agents
- ✅ Organizations prioritizing GitHub ecosystem integration
...
|---------|---------|---------------|-----------|-------------|------------|
| **`.prompt.md` files** | ✅ 1.93+ | ✅ 17.10+ | ✅ 2024.1+ | ✅ | ⚠️ PR Copilot only |
| **`.github/copilot-instructions.md`** | ✅ 1.93+ | ✅ 17.10+ | ✅ 2024.1+ | ❌ | ⚠️ PR Copilot only |
| **`.instructions.md` with `applyTo`** | ✅ 1.95+ | ✅ 17.10+ | ✅ 2024.2+ | ❌ | ❌ |
| **`.agent.md` files** | ✅ 1.106+ | ❌ | ❌ | ✅ | ❌ |
| **Agent handoffs** | ✅ 1.106+ | ❌ | ❌ | ❌ | ❌ |
| **User-scope prompts** | ✅ 1.93+ | ❌ | ⚠️ Via settings | ❌ | ❌ |
| **YAML frontmatter in prompts** | ✅ 1.93+ | ⚠️ Limited | ✅ 2024.1+ | ✅ | ❌ |
| **`tools` field in prompts** | ✅ 1.95+ | ❌ | ❌ | ❌ | ❌ |
| **`model` field in prompts** | ✅ 1.100+ | ❌ | ❌ | ⚠️ CLI-wide only | ❌ |
| **Background agents** | ✅ 1.107+ | ❌ | ❌ | ❌ | ❌ |
| **Cloud agents (GitHub PRs)** | ✅ 1.107+ | ❌ | ❌ | ❌ | ✅ |
| **BYOK (Bring Your Own Key)** | ✅ 1.102+ | ❌ | ❌ | ❌ | ❌ |
| **MCP server integration** | ✅ 1.102+ | ❌ | ❌ | ❌ | ❌ |
| **`AGENTS.md` support** | ✅ 1.100+ | ✅ 17.11+ | ✅ 2024.2+ | ✅ | ✅ |
| **Inline completion** | ✅ All | ✅ All | ✅ All | ❌ | ❌ |
| **Chat interface** | ✅ All | ✅ All | ✅ All | ✅ | ✅ |
...
|------------|---------|---------------|-----------|-------------|-------|
| `name` | ✅ | ✅ | ✅ | ✅ | Used as command name |
| `description` | ✅ | ✅ | ✅ | ✅ | Shown in UI |
| `agent` | ✅ 1.95+ | ❌ | ❌ | ❌ | `ask`, `edit`, `agent` modes |
| `model` | ✅ 1.100+ | ❌ | ❌ | ⚠️ | VS Code only |
| `tools` | ✅ 1.95+ | ❌ | ❌ | ❌ | Limit available tools |
| `argument-hint` | ✅ 1.96+ | ❌ | ❌ | ❌ | Help text for arguments |
| `variables` | ✅ 1.99+ | ❌ | ❌ | ❌ | Variable declarations |
...
| `model` | ✅ | ❌ | ❌ | ⚠️ | Model selection |
| `handoffs` | ✅ 1.106+ | ❌ | ❌ | ❌ | Workflow transitions |
| `target` | ✅ 1.107+ | ❌ | ❌ | ❌ | `vscode`, `github-copilot` |
| `mcp-servers` | ✅ 1.107+ | ❌ | ❌ | ❌ | Cloud agents only |

## Instruction File Syntax Compatibility

| YAML Field | VS Code | Visual Studio | JetBrains | Copilot CLI | Notes |
|------------|---------|---------------|-----------|-------------|-------|
| `applyTo` | ✅ 1.95+ | ✅ 17.10+ | ✅ 2024.2+ | ❌ | Glob pattern matching |
| `excludeAgent` | ✅ 1.106+ | ❌ | ❌ | ❌ | Agent exclusion list |
...
|----------|---------|---------------|-----------|-------|
| `#file` | ✅ | ⚠️ Limited | ❌ | Reference files in chat |
| `#fetch` | ✅ | ❌ | ❌ | Fetch remote content |
| `#selection` | ✅ | ✅ | ✅ | Current editor selection |
| `#editor` | ✅ | ⚠️ | ⚠️ | Active editor content |
| `#codebase` | ✅ 1.95+ | ❌ | ❌ | Entire codebase context |
...
|---------|---------|---------------|-----------|----------|
| **Enable custom instructions** | ✅ | ✅ | ✅ | IDE settings |
| **Enable workspace prompts** | ✅ Auto | ✅ Auto | ✅ Auto | Automatic discovery |
| **Enable user prompts** | ✅ | ❌ | ⚠️ Manual | User profile |
| **Model selection** | ✅ UI picker | ⚠️ Limited | ❌ | IDE-specific |
| **BYOK configuration** | ✅ UI + JSON | ❌ | ❌ | `languageModels.json` |
...
1. ✅ File is in `.github/prompts/`
2. ✅ Extension is `.prompt.md` (not `.prompt`)
3. ✅ YAML frontmatter is valid (parse with yamllint)
4. ✅ Test in VS Code: `/promptName` appears
5. ✅ Test in Visual Studio: `#promptName` works
6. ✅ Test in JetBrains: `/promptName` appears
7. ✅ Verify description shows in all UIs
8. ✅ Advanced fields gracefully ignored in older IDEs
```
