...
goal: "Document per-model-family prompt optimization differences for GPT, Claude, and Gemini"
rationales:
  - "Each model family has distinct structural and caching behaviors affecting prompt design"
  - "Official guides provide authoritative optimization strategies per provider"
...
**Purpose**: Quick-reference guide for optimizing prompts per model family. Each model is a different compiler — the same prompt produces different results depending on which model processes it. — the same prompt produces different results depending on which model processes it.
...
|---|---|---|---|---|
| **Claude (Opus, Sonnet)** | XML-tagged sections (`<instructions>`, `<context>`, `<examples>`) | Critical constraints first, nested XML for complex parts | Place static/reusable content at prompt start — 90% cached-prefix discount | Extended Thinking for complex tasks; set `budget_tokens`, don't micromanage |
| **GPT (4o, 5, o3)** | `developer` message for rules + `user` message for task | Numbered lists, Markdown headers, few-shot examples (2–5) | Place static instructions first in `developer` message — 50% cached-prefix discount | o3/o4-mini: high-level goals + success criteria only; NEVER "think step by step" |
...
- **Sources** (practical examples):
  - [08.00-how_to_optimize_prompts_for_specific_models.md](../../../03.00-tech/05.02-prompt-engineering/04-howto/08.00-how_to_optimize_prompts_for_specific_models.md) — GPT explicit instruction techniques, Claude clarity and context optimization (extended thinking, multishot), Gemini structured prompting (zero-shot vs few-shot), multi-model architecture patterns, model selection decision framework
  - [08.01-appendix_openai_prompting_guide.md](../../../03.00-tech/05.02-prompt-engineering/04-howto/08.01-appendix_openai_prompting_guide.md) — OpenAI-specific deep dive
  - [08.02-appendix_anthropic_prompting_guide.md](../../../03.00-tech/05.02-prompt-engineering/04-howto/08.02-appendix_anthropic_prompting_guide.md) — Anthropic-specific deep dive
  - [08.03-appendix_google_prompting_guide.md](../../../03.00-tech/05.02-prompt-engineering/04-howto/08.03-appendix_google_prompting_guide.md) — Google-specific deep dive
